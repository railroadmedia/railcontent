                ...
                <div x-ref="front"
                    ...
                    @if (!empty($bonus['logo']))
                        <div class="z-20 absolute bottom-0 left-0 right-0 px-4 py-3 text-center">
                            <img class="w-auto h-auto" style="max-height:55px;" src='{{ $bonus['logo'] }}'
                                alt="logo">
                        </div>
                        <div class="inset-0 absolute z-10"
                            style="background:linear-gradient(to bottom, transparent 66%, rgba(0,0,0,0.75));"></div>
                    @endif
                </div>
...
@foreach ($products as $productItem)
    <div class="bonus-wrap relative inline-block align-top mx-auto mb-4 px-1 md:px-3 @if (!empty($bonusWidth)) {{ $bonusWidth }} @else w-1/2 md:w-1/4 lg:w-1/5 @endif"
        ...
        x-on:click="
                    ...
                    ">
        <div class="flip-div inline-block relative w-full group"
            style="@if (empty($bigCard)) padding-bottom: 133%; @else padding-bottom: 103%; @endif perspective: 1000px;">
            <div class="text-center w-full h-full absolute cursor-pointer" style="transform-style: preserve-3d;">
                <div x-ref="front"
                    class="border-2 border-promo front absolute z-20 overflow-hidden rounded-xl w-full h-full transition-transform duration-700"
                    style="@if (!empty($special)) overflow: visible;border-color: #cda880; @endif backface-visibility: hidden;">
                    @if (!empty($badge))
                        <h6
                            class="absolute text-white top-0 left-0 w-full py-0.5 bg-{{ $theme }} font-bebas uppercase">
                            {{ $badge }}</h6>
                    ...
                    @if (!empty($productItem['thumbnail_logo']))
                        <div class="z-20 absolute bottom-0 left-0 right-0 px-4 py-3 text-center">
                            <img class="w-auto h-auto" style="max-height:55px;" src='{{ $productItem['thumbnail_logo'] }}'
                                alt="logo">
                        </div>
                        <div class="inset-0 absolute z-10"
                            style="background:linear-gradient(to bottom, transparent 66%, rgba(0,0,0,0.75));"></div>
                ...
                <div x-ref="back"
                    ...
                    <div class="w-full h-full mx-auto text-center text-white flex flex-wrap justify-center items-center content-center p-2 md:p-3"
                        style="background:linear-gradient(to bottom, #01050f, #021225);">
                        <p class="leading-normal mx-auto text-sm">{!! $productItem['short_desc'] !!}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <p class="w-full leading-normal mt-2">
            <span style="text-transform:uppercase; display:inline-block;">
                @if (!empty(floatval($productPrices[$productItem['sku']]->price)))
                    <s class="opacity-40">${{ floatval($productPrices[$productItem['sku']]->price) }}</s>
                @endif
                <strong class="text-promo">FREE</strong>
            </span><br>
            <em>
