            ...
            <div class="flex flex-wrap items-start justify-center text-left w-full sm:w-1/2">
                @foreach ($gridItems as $key => $gridItem)
                    ...
                    <div class="flex flex-wrap items-start w-full sm:px-3 mb-5 sm:mb-8" x-data="{ open: false }">
                        <div class="pb-[80%] @if(empty($gridItem['big'])) sm:pb-[60%] @else sm:pb-[80%]  @endif overflow-hidden text-white relative w-full rounded-xl"
                        ...
                        >
                            <picture class="absolute inset-0 w-full h-full object-cover bg-center rounded-xl" style="background: linear-gradient(180deg, transparent, rgba(246, 248, 252, 0.9));">
                                ...
                                <img x-ref="image"
                                    ...
                                    @if(!empty($gridItem['imageM']))
                                        data-src="https://d21q7xesnoiieh.cloudfront.net/fit-in/980x0/filters:quality(95)/{{ $gridItem['imageM'] }}"
                                    @else
...
{{--                                    <h5 class="rounded-full border-2 border-{{ $theme }} inline-block w-9 h-9 leading-8 mx-0 text-center flex-grow-0 flex-shrink-0"><strong>@if($theme == 'musora'){{ $key + 2 }}@else{{ $key + 1 }}@endif</strong></h5>--}}
            ...
            @if(!empty($full))
                @foreach ($gridItems as $key => $gridItem)
                    @if(!empty($gridItem['full']))
                        <div class="flex flex-wrap items-start w-full sm:px-3 mb-7 sm:mb-14" x-data="{ open: false }">
                            <div class="pb-[80%] sm:pb-96 overflow-hidden text-white relative w-full rounded-xl"
                                :class="{'opacity-0': !lazyLoad, 'opacity-100': lazyLoad}"
                            ...
                            >
                                <picture>
                                    ...
                                    <img
                                        @if(!empty($gridItem['imageM']))
                                            src="https://d21q7xesnoiieh.cloudfront.net/fit-in/0x470/filters:quality(95)/{{ $gridItem['imageM'] }}"
                                        @else
                                            src="https://d21q7xesnoiieh.cloudfront.net/fit-in/0x470/filters:quality(95)/{{ $gridItem['image'] }}"
                                        @endif
                                        alt="{{ $gridItem['title'] }}"
                                        class="absolute inset-0 w-full h-full object-cover object-top rounded-xl opacity-0 transition-opacity"
