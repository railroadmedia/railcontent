                    ...
                    @foreach ($images as $key => $image)
                        @if((!empty($videoSrc) && $key !== 0) || empty($videoSrc) )
                                <div @if(count($images) > 1)style="display: none;"@endif>
                                    <img
                                        class="w-full"
                                        src="https://www.musora.com/musora-cdn/image/width=1000,quality=85/@if(str_contains($image->path, 'amazonaws') || str_contains($image->path, 'cloudfront') || str_contains($image->path, 'vimeocdn')){{$image->path}}@else{{'https://laravel-nova.s3.us-east-2.amazonaws.com/'.$image->path}}@endif"
