...
@if(empty($weekDescription)) no-drop @endif">
    ...
    @if(!empty($description))
        <div class="py-3 px-2 sm:px-3 ml-auto">
            <i class="text-sm sm:text-xl @if(!empty($navyBorder)) text-navy-600 @endif fas fa-chevron-down transform transition-all duration-300 @if(!empty($defaultOpen)) rotate-180 @endif"></i>
        </div>
    @endif
</div>
