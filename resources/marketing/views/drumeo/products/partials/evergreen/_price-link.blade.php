...
<div class="w-full pt-4">
    @if (!empty($price) || !empty($enrollmentLink) || !empty($brandTitle))
        @if ($currentDate->isSameDay($blackFridayDate))
            <strong> Only <span class="line-through">{{ $price }}</span> {{$discountedPrice}} For Black Friday (Save 61%)</strong>
        @elseif ($currentDate->isSameDay($cyberMondayDate))
            <strong> Only <span class="line-through">{{ $price }}</span> {{$discountedPrice}} For Cyber Monday (Save 61%)</strong>
