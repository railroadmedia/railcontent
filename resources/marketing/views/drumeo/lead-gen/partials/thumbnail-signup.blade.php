<a {{ !empty($url) ? 'href=' . $url : '' }}  {{ !empty($modal) ? 'data-open=' . $modal : 'data-open=signUpModal' }} class="lesson-grid-item cursor-pointer {{ !empty($modal) ? 'autoplay-video' : '' }} {{ !empty($locked) ? 'locked' : '' }}">
