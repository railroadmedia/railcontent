<?php
$bonusVideos = [
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-bottom/nitro-max-new.webp',
        'header' => 'Alesis Nitro Max E-Kit | Drumeo Edition',
        'description' => '<p>Transform your room, apartment or townhouse into a drum studio.</p><p class="my-2">Drumeo partnered with Alesis to offer you the highest-rated beginner e-kit with one year of unlimited drum lessons. It\'s everything you need to play the drums. And with mesh heads, you can practice late without disturbing anyone.</p>',
        'price' => 599,
        'badge' => null,
        'extraBadge' => 'Only Discounted With Full Bundle',
        'sku' => 'alesis-nitro-max-e-kit'
    ],

    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/AM.webp',
        'header' => 'Drumeo Annual Membership',
        'description' => '<p>Learn the drums with the world\'s best teachers, step-by-step lessons, and detailed song breakdowns.</p><p class="my-2">You\'ll have everything you need to learn beginner beats, drum set independence, new styles and beyond. It\'s the ultimate way to get started or get back into playing the drums.</p>',
        'price' => 240,
        'vimeoId' => '898623255',
        'sku' => 'DLM-1-year'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/AM.webp',
        'header' => 'Drumeo Annual Membership',
        'description' => '<p>Learn the drums with the world\'s best teachers, step-by-step lessons, and detailed song breakdowns.</p><p class="my-2">You\'ll have everything you need to learn beginner beats, drum set independence, new styles and beyond. It\'s the ultimate way to get started or get back into playing the drums.</p>',
        'offerPrice' => '<strong><span class="opacity-30 line-through">$240</span><span class="font-black"> $140</span></strong> <span class="text-xs">for your first year</span>',
        'vimeoId' => '898623255',
        'sku' => 'DLM-1-year-drumeo'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/AM.webp',
        'header' => 'Drumeo Annual Membership',
        'description' => '<p>Learn the drums with the world\'s best teachers, step-by-step lessons, and detailed song breakdowns.</p><p class="my-2">You\'ll have everything you need to learn beginner beats, drum set independence, new styles and beyond. It\'s the ultimate way to get started or get back into playing the drums.</p>',
        'badge' => null,
        'price' => 240.00,
        'vimeoId' => '898623255',
        'sku' => 'DLM-1-year-kit'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/30DD.webp',
        'header' => '30-Day Drummer',
        'description' => '<p>Learn the drums by playing the drums.</p><p class="my-2">In 30-Day Drummer, you\'ll build your timing & coordination by following daily guided workouts.</p><p>By the end of the month you\'ll have the skills to play hundreds of popular songs on the drums.</p>',
        'badge' => null,
        'price' => 127.00,
        'vimeoId' => '884916532',
        'sku' => '30-day-drummer-4'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/30DI.webp',
        'header' => '30-Day Independence',
        'description' => '<p>Independence is the hardest part of learning the drums.</p><p class="my-2">In 30-Day Independence, you\'ll build your 4-way coordination slowly by following daily guided workouts with El Estepario Siberiano.</p><p>In 30 days you\'ll drastically improve your drum set independence.</p>',
        'badge' => null,
        'price' => 127.00,
        'vimeoId' => '975466470',
        'sku' => '30-day-independence'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/30DI.webp',
        'header' => '30-Day Independence',
        'description' => '<p>Independence is the hardest part of learning the drums.</p><p class="my-2">In 30-Day Independence, you\'ll build your 4-way coordination slowly by following daily guided workouts with El Estepario Siberiano.</p><p>In 30 days you\'ll drastically improve your drum set independence.</p>',
        'badge' => '$42.33',
        'price' => 127.00,
        'vimeoId' => '975466470',
        'sku' => '30-day-independence-challenge'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/30DDB.webp',
        'header' => '30-Day Double Bass',
        'description' => '<p>For the metalheads 🤘</p><p class="my-2">Learn double bass speed, endurance and coordination with daily guided workouts.</p><p>You\'ll play along with your teacher, 66Samus, for 30 days to improve your foot speed and control.</p>',
        'badge' => null,
        'price' => 127.00,
        'vimeoId' => '1010405112',
        'sku' => '30-day-double-bass'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/30DDB.webp',
        'header' => '30-Day Double Bass',
        'description' => '<p>For the metalheads 🤘</p><p class="my-2">Learn double bass speed, endurance and coordination with daily guided workouts.</p><p>You\'ll play along with your teacher, 66Samus, for 30 days to improve your foot speed and control.</p>',
        'badge' => '$42.33',
        'price' => 127.00,
        'vimeoId' => '1010405112',
        'sku' => '30-day-double-bass-challenge'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/30DJ.webp',
        'header' => '30-Day Jazz',
        'description' => '<p>Immerse yourself in drumming\'s pioneering style.</p><p class="my-2">In 30-Day Jazz, you\'ll be introduced to the grooves, fills and techniques you need to play along with iconic jazz songs.</p><p>This course is taught by Ulysses Owens Jr. - an acclaimed instructor at the Juilliard School.</p>',
        'badge' => null,
        'price' => 127.00,
        'vimeoId' => '1018759800',
        'sku' => '30-day-jazz'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/30DC.webp',
        'header' => '30-Day Chops',
        'description' => '<p>Learn blistering linear chops in 30 days.</p><p class="my-2">30-Day Chops teaches you challenging linear chops by adding a few notes each day.</p><p>By the end of the month, you\'ll be playing explosive fills + have the tools to create your own chops.</p>',
        'badge' => null,
        'price' => 127.00,
        'vimeoId' => '884916500',
        'sku' => '30-day-chops'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/30DC.webp',
        'header' => '30-Day Chops',
        'description' => '<p>Learn blistering linear chops in 30 days.</p><p class="my-2">30-Day Chops teaches you challenging linear chops by adding a few notes each day.</p><p>By the end of the month, you\'ll be playing explosive fills + have the tools to create your own chops.</p>',
        'badge' => '$42.33',
        'price' => 127.00,
        'vimeoId' => '884916500',
        'sku' => '30-day-chops-challenge'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-bottom/p4-practice-new.webp',
        'header' => 'Drumeo P4 Practice Pad',
        'description' => '<p>The P4 Practice Pad is the most versatile practice pad in the world – with four different playing surfaces to replicate different parts of the drum set. </p><p class="my-2">When you’re running rudiments & stickings at the pad, you’re still getting the various feels of your drum set.</p>',
        'badge' => null,
        'price' => 79.00,
        'sku' => 'practicepad'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-bottom/quietkick-new.webp',
        'header' => 'Drumeo QuietKick',
        'description' => '<p>The QuietKick includes everything you need to start working out your foot.</p><p class="my-2"> You’ll get three strike pads, one QuietKick unit, and one reverse-angle beater (with the option to add a second beater if you play double-kick).</p>',
        'badge' => null,
        'price' => 79.00,
        'sku' => 'quietkick'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-bottom/padstand-new.webp',
        'header' => 'Drumeo PadStand',
        'description' => '<p>Practice anywhere with perfect height and ergonomics.</p><p class="my-2">Works with ALL 12” practice pads.</p><p>The Drumeo PadStand can be used with a QuietPad, or any other 12” or smaller practice pad. Simply tighten the basket for the perfect fit.</p>',
        'badge' => null,
        'price' => 79.00,
        'sku' => 'padstand'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/drumeo-sticks.webp',
        'header' => '1 Pair Of Drumsticks',
        'description' => '<p>Our new Drumeo 5A Drumsticks by Vater – made of hickory for strength and durability, featuring up to 2X the moisture content than most manufacturers for longer lasting sticks, and personally hand-rolled by Alan Vater to ensure they’re weighted and tone-matched to perfection.</p>',
        'badge' => null,
        'price' => 12.95,
        'sku' => 'Drumeo-VaterSticks'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/drumeo-sticks.webp',
        'header' => '2 Extra Pairs of Drumsticks',
        'description' => '<p>Our new Drumeo 5A Drumsticks by Vater – made of hickory for strength and durability, featuring up to 2X the moisture content than most manufacturers for longer lasting sticks, and personally hand-rolled by Alan Vater to ensure they’re weighted and tone-matched to perfection.</p>',
        'badge' => null,
        'price' => 12.95,
        'sku' => 'Drumeo-VaterSticks-2'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-bottom/drumeo-eardrums-new.webp',
        'header' => 'Drumeo EarDrums',
        'description' => '<p>Protect your ears and play your favorite songs.</p><p class="my-2">The Drumeo EarDrums have helped 14,000+ drummers practice, tour, and listen to music in all different situations. Now they’re back in black with upgraded cables and a custom hard case.</p><p>You’ll never leave home without them.</p>',
        'badge' => null,
        'price' => 149.00,
        'sku' => 'eardrums'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/drumeo-headphones.webp',
        'header' => 'Drumeo Headphones',
        'description' => '<p>The perfect headphones for your E-kit.</p><p class="my-2">The Drumeo Headphones deliver studio-quality sound, all-day comfort, and improved low-end for E-Kit bass frequencies.</p><p>That means you can practice longer without hurting your ears.</p>',
        'badge' => null,
        'price' => 99.00,
        'sku' => 'headphones'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/best-beginner-new.webp',
        'header' => 'The Best Beginner Drum Book',
        'description' => '<p>The simplest guide for beginner drummers to get started on the drums and take their drumming to the next level.</p><p class="my-2">Getting started on the drums just became easier...</p>',
        'badge' => null,
        'price' => 29.99,
        'sku' => 'BeginnerBook'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/easy-rudiments.webp',
        'header' => 'Easy Rudiments Book',
        'description' => '<p>The 15 Rudiments You Actually Need To Know (And How To Learn Them Quickly).</p><p class="my-2">Easy Rudiments is your simplified guide to learning the 15 rudiments you’ll actually use on a drum set.</p><p>Plus, you’ll know HOW and WHY to learn them in the first place.</p>',
        'badge' => null,
        'price' => 29.99,
        'sku' => 'easy-rudiments-book'

    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/drummers-toolbox-new.webp',
        'header' => 'The Drummer’s Toolbox Book',
        'description' => '<p>The Drummer’s Toolbox presents drummers of all skill levels with the most comprehensive introduction to 101 drumming styles from the past century.</p><p class="my-2">Expand your musical vocabulary and become more versatile behind the drum set.</p>',
        'badge' => null,
        'price' => 29.99,
        'sku' => 'the-drummers-toolbox-book'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/gift-bundle/AP.webp',
        'header' => 'Drumeo One-Year Access Pass',
        'description' => '<p>The one-year access pass for gifting a full year of lessons without an ongoing subscription.</p> <p class="my-2">You’ll get a physical access pass that you, or anyone, can redeem anytime – loaded with the lessons they’ll love to reach all of their goals on the drums.</p>',
        'offerPrice' => '<strong class="font-black">$240</strong>',
        'badge' => null,
        'vimeoId'=>'898623255',
        'sku' => 'drumeo-1-year'
    ],
    [
        'image' => 'marketing/drumeo/promos/november/2024/bonuses-new/quietpad.webp',
        'header' => 'QuietPad',
        'description' => '<p>Practice anywhere with two full-size playing surfaces.</p><p class="my-2">The portable, double-sided practice pad with different volumes so you can practice late into the night.</p>',
        'badge' => null,
        'price' => 59,
        'vimeoId' => '982740966',
        'sku' => 'quietpad'
    ],
];
