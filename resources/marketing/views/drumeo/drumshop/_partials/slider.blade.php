                ...
                @if (!empty($images))
                    @foreach ($images as $key => $image)
                        <div style="display: none;">
                            <img class="w-full" src="https://cdn.musora.com/image/fetch/w_1000,q_auto:best/{{ $image->path }}" alt="slide image {{ $key+1 }}">
                        </div>
                    @endforeach
                @endif
            </div>
                ...
                @if(!empty($images))
                    @foreach ($images as $image)
                        @if((!empty($videoSrc) && $key !== 0) || empty($videoSrc) )
                            <div @if(count($images) > 1)style="display: none;"@endif><img class="w-full rounded border-4 border-solid w-full" style="border-color: #FFF;" src="https://cdn.musora.com/image/fetch/w_1000,q_auto:best/{{ $image->path }}" alt="slide image {{ $key+1 }}"></div>
                        @endif
                    @endforeach
                @endif
            </div>
