<div class="product-wrap lg:tw-w-2/3 tw-px-3 md:tw-px-4">
    <div class="pack-details slider-wrap tw-mb-1 md:tw-pt-9 md:tw-mt-2 lg:tw-pt-11 lg:tw-mt-5">
        <div class="tw-text-center tw-pb-5 sm:tw-pb-6 lg:tw-pb-7">
            @if(!empty($headerText))
        ...
        <div class="slider-container tw-overflow-hidden tw-w-full tw-mb-5 md:tw-mb-7" style="font-size: 0;@if(!empty($interactiveBanner)) max-height:1500px @endif">
            <div class="slider-for tw-overflow-hidden @if(!empty($interactiveBanner) || !empty($packBanner)) tw-rounded-t @else tw-rounded @endif">
                @if (!empty($videoUrl))
                    <div class="tw-overflow-hidden tw-relative tw-w-full" style="padding-bottom: 56.25%;">
                        <iframe class="tw-absolute tw-w-full tw-h-full tw-inset-0" src="{{ $videoUrl }}" frameborder="0" allowfullscreen id="videoPlayer"></iframe>
                        <div class="tw-hidden" id="videoSrc">{{$videoUrl}}</div>
                    </div>
                @endif
                @if (!empty($slideImages))
                    @foreach ($slideImages as $slideImage)
                        <div style="display: none;">
                            <img class="tw-w-full" src="https://cdn.musora.com/image/fetch/w_1000,q_auto:best/{{ $slideImage }}">
                        </div>
                    @endforeach
                @endif
            </div>
            <div class="slider-nav tw-mx-auto tw-w-full @if(!empty($noSlider)) tw-hidden @endif">
                @if(!empty($slideImages))
                    @if(!empty($videoUrl))
                        <div style="display: none;"><img class="tw-w-full" src="https://cdn.musora.com/image/fetch/w_1000,q_auto:best/{{ $defaultCover  }}"></div>
                    @endif
                    @foreach ($slideImages as $slideImage)
                        <div style="display: none;"><img class="tw-w-full" src="https://cdn.musora.com/image/fetch/w_1000,q_auto:best/{{ $slideImage }}"></div>
                    @endforeach
                @endif
            </div>
