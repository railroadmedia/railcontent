<div class="dropdown text-center border-2 border-white rounded-lg overflow-hidden flex cursor-pointer mb-3 select-none
...
@if(empty($weekDescription)) no-drop @endif">
    ...
    <div class="p-3 text-left flex-grow relative">
        ...
        @if(!empty($date)) <p class="hidden sm:inline-block absolute right-0 top-1/2" style="transform: translate(0, -50%);"><em><strong> {!!  $date !!} </strong></em></p> @endif
        @if(!empty($description))
            <p class="description transition-all duration-300 text-xs sm:text-sm"><br>{!! nl2br( $description) !!}</p>
        @endif
    </div>
    @if(!empty($description))
        <div class="py-3 px-2 sm:px-3 ml-auto text-sm sm:text-xl @if(!empty($customArrow)) {{ $customArrow }} @endif">
