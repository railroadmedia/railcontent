    ...
    <div class="text-center">
        ...
        <div class="bg-white text-black overflow-hidden rounded-2xl block mx-auto mb-4 lg:mb-0 group border-2 @isset($productTheme) @if($productTheme != 'drumeo') border-{{ $productTheme }} @endif @endisset">
            <div class="px-3 py-6 md:py-7 text-center">
                <h3 class="leading-tight mb-2"><strong>{{ $cardTitle ?? '' }}</strong></h3>
                ...
                @isset($cardButtons)
                    <div class="flex items-center justify-center">
                        @foreach($cardButtons as $button)
                            <a href="{{ $button['link'] ?? '#' }}" aria-label="{{ $button['text'] ?? '' }}" class="mx-1 join bg-{{ $productTheme }} smaller w-1/2 transition-opacity duration-300 group-hover:opacity-80 max-w-[230px]">{{ $button['text'] ?? '' }}</a>
                        @endforeach
                    </div>
                @endisset
            </div>
