                ...
                <div class="flex items-center justify-center">
                    @foreach($cardButtons as $button)
                        @if ($products[$sku]->getStockAvailability() > 1 && !empty($products[$sku]->getStockAvailability()))
                            <a href="{{ $button['link'] ?? '#' }}" aria-label="{{ $button['text'] ?? '' }}" class="mx-1 join bg-{{ $productTheme }} smaller w-2/3 transition-opacity duration-300 group-hover:opacity-80 max-w-[230px]">{{ $button['text'] ?? '' }}</a>
                        @else
