@extends('pianote.lead-gen.riffs-and-fills.pages.lesson-page')

@section('sub-title', 'Combining And Applying Your Skills')

@section('video', '//player.vimeo.com/video/415195897')

@section('current-lesson-number', 5)

@section('lesson-description')
    <a class="join smaller mb-4" target="_blank" href="https://d1923uyy6spedc.cloudfront.net/255250-resource-1588935038.pdf">DOWNLOAD PDF</a>
    <p>Learn how to take what you've learned so far and put it together in the context of a song. This is where all the magic comes to life!
    <br><br>
    <strong>C Chord Inversions</strong><br><br></p>
    <img src="https://d1923uyy6spedc.cloudfront.net/255253-sheet-image-1588760291.svg">
    <p><br><br><strong>G Chord Inversions</strong><br><br></p>
    <img src="https://d1923uyy6spedc.cloudfront.net/255254-sheet-image-1588760839.svg">
    <p><br><br><strong>Am Chord Inversions</strong><br><br></p>
    <img src="https://d1923uyy6spedc.cloudfront.net/255255-sheet-image-1588762212.svg">
    <p><br><br><strong>F Chord Inversions</strong><br><br></p>
    <img src="https://d1923uyy6spedc.cloudfront.net/255256-sheet-image-1588762774.svg">
@endsection

@section('previous')
    /riffs-and-fills/lessons/4
@endsection

@section('next')
    /riffs-and-fills/lessons/6
@endsection

@section('prev-thumb', 'https://d1923uyy6spedc.cloudfront.net/255248-card-thumbnail-maxres-1588771051.jpg')

@section('next-thumb', 'https://d1923uyy6spedc.cloudfront.net/255250-card-thumbnail-maxres-1588771138.jpg')
