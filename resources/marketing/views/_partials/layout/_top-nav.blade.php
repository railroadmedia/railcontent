<nav id="nav" class="@if(!empty($whiteNav)) bg-white @else bg-[#020815] @endif shadow-md fixed w-full top-0 left-0 flex items-center h-10 md:h-14 top-bar z-50">
    ...
    <div class="ml-auto flex items-center">
        <a
            @if(!empty($scrollToJoin))
                ...
                class="btn-primary btn-small text-base leading-none text-black bg-musora border-0 pb-1.5 px-3 md:pb-3 md:pt-4 md:px-8 mr-1.5 mb-0 h-auto md:h-initial anchor-slide"
            @else
                href="/choose-plan"
                class="btn-primary btn-small text-base leading-none text-black bg-musora border-0 pb-1.5 px-3 md:pb-3 md:pt-4 md:px-8 mr-1.5 mb-0 h-auto md:h-initial"
            @endif
        >
    ...
    <!-- Sidebar Toggle -->
    <button class="@if(!empty($whiteNav)) invert @endif text-white h-10 md:h-14 relative focus:outline-none @if(!empty($hideMenu)) opacity-0 px-0.5 @else w-10 md:w-14 @endif" x-on:click="sidebarOpen = !sidebarOpen, showOverlay = !showOverlay">
