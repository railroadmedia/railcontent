<section x-show="filter === 'all'">
    ...
    <div class="container mx-auto">
        <h5 class="leading-tight mb-4 md:mb-5"><strong><i class="fas fa-fire text-{{ $brand }} mr-1"></i> {!!  $header  !!}</strong></h5>
                            ...
                            <ul class="splide__list">
                                @foreach ($bundles as $key => $bundle)
                                    <li class="splide__slide flex flex-col items-center justify-start">
                                        <a href="{{ $bundle['slug'] }}" class="w-full mx-auto p-1 sm:p-2 @if(!empty($bundle['specialW'])) lg:w-2/3 @elseif(!empty($bundle['specialW2'])) lg:w-1/3 @elseif(!empty($bundle['full'])) @else sm:w-1/2 @endif">
