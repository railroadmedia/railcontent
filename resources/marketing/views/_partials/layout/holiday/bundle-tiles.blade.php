    ...
    <div class="container mx-auto">
        ...
        <div class="flex flex-wrap mb-5 sm:mb-10">
            @foreach(array_filter($bundles, function($val){if($val['visible'] === 1){ return 1; }}) as $key => $bundle)
                <a href="{{ $bundle['slug'] }}" class="w-full @if(empty($bundle['full'])) sm:w-1/2 @endif mx-auto p-1 sm:p-2">
                    <div class="flex items-center @if(empty($bundle['full'])) justify-center @endif w-full overflow-hidden relative text-white rounded-xl px-5 lg:px-16 xl:pl-20 py-5 sm:py-7 lg:py-8"
