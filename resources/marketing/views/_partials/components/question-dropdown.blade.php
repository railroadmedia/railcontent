    ...
    <div class="flex">
        <div class="text-left flex-grow relative">
            ...
            >
                ...
                >
                    @if(!empty($num))
                        <span class="rounded-full px-2 md:px-2.5 min-w-0 py-1 text-xs md:text-sm text-white absolute -left-11 sm:-left-14 md:-left-16 -top-0.5 sm:top-0.5"
                            :class="open ? 'bg-{{$theme}}' : 'bg-[#838C98]'">{!! $num !!}</span>
                    @endif
                    {!! $title !!}
                </h5>
            </div>
            ...
            @foreach($lessonInfo as $key => $info)
                <div class="bg-white @if(!empty($info['desc'])) p-4 @endif rounded-xl flex flex-col md:flex-row mb-2 @if(empty($info['desc'])) md:items-center @endif">
                    <img
                        ...
                        src="https://www.musora.com/cdn-cgi/image/width=650,quality=95/{{$info['thumb']}}"
