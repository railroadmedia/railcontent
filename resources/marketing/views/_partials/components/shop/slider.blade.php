        ...
        <div class="slider-container overflow-hidden w-full mb-5 md:mb-7" style="font-size: 0;@if(!empty($interactiveBanner)) max-height:1500px @endif">
                    ...
                    @foreach ($images as $key => $image)
                        @if((!empty($videoSrc) && $key !== 0) || empty($videoSrc) )
                            <div style="display: none;">
                                <img class="w-full" src="https://www.musora.com/cdn-cgi/image/width=1400,quality=95/{{$image->path}}" alt="slide image {{ $key+1 }}">
                            </div>
                        @endif
                    @endforeach
                @endif
            ...
            <div class="slider-nav mx-auto w-full @if(!empty($images) && count($images) <= 1) hidden @endif">
                @if(!empty($images))
                    @foreach ($images as $image)
                        @if((!empty($videoSrc) && $key !== 0) || empty($videoSrc) )
                            <div @if(count($images) > 1)style="display: none;"@endif><img class="w-full rounded border-4 border-solid w-full" style="border-color: #FFF;" src="https://www.musora.com/cdn-cgi/image/width=1400,quality=95/{{$image->path}}" alt="slide image {{ $key+1 }}"></div>
                        @endif
                    @endforeach
                @endif
            </div>
            @if(!empty($interactiveBanner))
                <div class="rounded-b p-4 flex justify-between items-center bg-drumeo sm:py-1 sm:px-5">
                    <img class="w-full sm:w-2/3" src="https://www.musora.com/cdn-cgi/image/width=1400,quality=95/https://s3.amazonaws.com/drumeo-packs/interactive-edition-logo-white.png">
                    <img class="w-40 hidden sm:inline lg:w-48" src="https://www.musora.com/cdn-cgi/image/width=1400,quality=95/{{ $interactiveBanner }}">
                </div>
            @elseif(!empty($packBanner))
                <div class="rounded-b p-4 w-full flex justify-between items-center bg-drumeo sm:py-1 sm:px-5">
                    <img class="w-full sm:w-2/3" src="https://www.musora.com/cdn-cgi/image/width=1400,quality=95/https://s3.amazonaws.com/drumeo-packs/drumeo-training-pack-white.png">
