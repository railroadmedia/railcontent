<div class="file-field" x-bind:disabled="disabled">
    <label x-bind:for="id" 
    ...
    >
        ...
        <input type="file" 
                ...
                x-on:change="checkUploadedFile($event)">
    </label>
    <!-- Message -->
    <div class="input-messages">
        <p class="primary-message text-[#00101D] dark:text-[#9EC0DC]">{{ messageController }}</p>
        <p x-if="this.formState !== 2" class="secondary-message">Max File Size {{ megabiteLimit }}MB</p>
    </div>
    <!-- Uploadedd Files -->
    <div x-if="files.length !== 0">
        <div x-for="(file, index) in files"
                x-bind:key="index" 
                class="bg-green-50 p-2 inline-flex items-center font-bold text-xs w-full mb-2"
        >
            <span class="truncate">{{ files[index].name }}</span> 
            <span class="ml-1 text-gray-500 mr-auto whitespace-nowrap">
                ({{ formatBytes(files[index].size) }})
            </span>
            <!-- Remove File -->
            <div class="ml-4 inline-flex"
                    x-on:click="removeFile(index)">
                <svg xmlns="http://www.w3.org/2000/svg" class="text-gray-500 h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
