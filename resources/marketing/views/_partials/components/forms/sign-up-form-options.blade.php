...
<form id="{{$cleanFormId}}" accept-charset="UTF-8" method="POST"
    ...
    class="ajax-form clearfix facebook-track-lead mx-auto">
            ...
            @foreach ($checkboxItems as $label => $id)
                <div class="relative flex items-start justify-center lg:items-center lg:justify-start lg:space-x-3 mb-2 lg:mb-0">
                    <div class="flex h-6 items-center">
                        <input id="{{ $id }}" aria-describedby="{{ $id }}-description" type="checkbox" class="instrument-checkbox cursor-pointer h-4 w-4 rounded border-gray-300 text-{{$theme}} focus:ring-{{$brand}}" value="{{ $label }}" />
                    </div>
            ...
            axios.post(form.action, formData)
                .then(response => {
                    if (response.status === 201) {
                        ...
                        document.querySelector('.thank-you-box').classList.remove('invisible', 'max-h-0', 'opacity-0');
                        setTimeout(() => {
