<div @if(!empty($assignmentID)) id="{{ $assignmentID }}" @endif class="assignment-row border-gray-100 border-b-2 py-4 sm:px-3" style="border-bottom-style: solid;">
    <div class="w-full flex text-left items-center justify-between">
        ...
        @elseif(!empty($pdfURL))
            <i class="fal fa-fw fa-file-music text-3xl flex-shrink-0 text-black"></i>
        @elseif(!empty($mp3URL))
            <a href="{{ $mp3URL }}" target="_blank"><i class="fal fa-fw fa-music text-3xl text-black"></i></a>
        @elseif(!empty($zipURL))
            <a href="{{ $zipURL }}" target="_blank"><i class="fal fa-fw fa-cloud-download text-3xl text-black"></i></a>
        @endif
    ...
    @if(!empty($pdfURL) || !empty($mp3URL))
        <div class="w-full dropdown px-2 invisible opacity-0 h-auto max-h-0 overflow-hidden transition-all duration-300">
            ...
            @if(!empty($pdfURL))
                <img class="mt-4" src="https://cdn.musora.com/image/fetch/w_1300,q_auto:best/{{ $pdfURL }}">
            @endif
        </div>
    @endif
</div>
