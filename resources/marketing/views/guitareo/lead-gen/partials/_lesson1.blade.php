<section class="text-white text-center clearfix px-3 md:px-4 py-7 md:py-12 lg:py-16" style="@isset($bg)background:{{ $bg }}; @endisset @isset($bgImg) background-image:url(https://www.musora.com/cdn-cgi/image/width=1500,quality=95/{{$bgImg}}); @endisset">
            ...
            @foreach($lessons as $key => $lesson)
                <div class="flex w-5/6 sm:flex-auto md:w-1/2 lg:w-1/3 px-2 md:px-3 mb-7 md:mb-10 step">
                    <div class="rounded-xl overflow-hidden" @isset($textbg) style="background: {{ $textbg }}; "@endisset>
                        <img class="w-full" src="https://www.musora.com/cdn-cgi/image/width=450,quality=95/{{ $lesson['image'] }}" alt="lesson-{{ $key+1 }}">
