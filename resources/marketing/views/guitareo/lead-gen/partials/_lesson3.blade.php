            ...
            @foreach ($lessons as $key => $lesson)
                <div class="w-5/6 md:w-1/2 lg:w-1/3 px-2 md:px-4 mb-7 md:mb-10 step">
                    <div class="@isset($lesson['playButton'])cursor-pointer play-vimeo autoplay-video @endisset relative" @isset($lesson['dataOpen']) data-open="{{ $lesson['dataOpen'] }}" @endisset>
                        <picture>
                            <source media="(min-width: 1024px)" srcset="https://www.musora.com/cdn-cgi/image/width=450,quality=95/{{ $lesson['image'] }}">
                            <source media="(min-width: 768px)" srcset="https://www.musora.com/cdn-cgi/image/width=500,quality=95/{{ $lesson['image'] }}">
                            <img class="w-full rounded-lg lazyload" data-src="https://www.musora.com/cdn-cgi/image/width=520,quality=95/{{ $lesson['image'] }}" alt="lesson-thumbnail{{ $key + 1 }}">
                        </picture>
