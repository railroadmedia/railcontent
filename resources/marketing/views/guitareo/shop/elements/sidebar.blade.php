<div class="side-bar sliding-function px-3 md:px-4 lg:px-4 lg:w-1/3 lg:mt-5">
    ...
    <div class="side-slide overflow-hidden md:rounded md:border md:border-solid" style="border-color: #CCD3D3;">
        {{--<div class="promo-tab hidden lg:block" style="position:relative;background:#1e1e1e;">--}}
            {{--<img src="https://guitareo.s3.amazonaws.com/sales/promos/black-friday/xm-logo2021.png">--}}
        {{--</div>--}}
        ...
        <div class="buy-section active px-5 pt-2 pb-6 text-center lg:py-6">
            ...
            @endif
            @if(!empty($logo))
                <img class="mx-auto mb-4 w-auto max-h-28 hidden lg:inline" src="{{ $logo }}">
            @endif
            ...
            @if(isset($fullPrice) && isset($price) && ($fullPrice - $price) > 0)
                ...
                <div class="text-center leading-none text-3xl uppercase md:text-4xl"><s class="opacity-40 text-2xl">${{ $fullPrice }}</s>
                    @if(number_format($price, 2) == intval($price))
                        <strong class="text-guitareo">$<span class="chosen-variant-price-float">{{  $price  }}</span></strong>
                    @else
                        <strong class="text-guitareo">$<span class="chosen-variant-price-float">{{  number_format($price, 2)  }}</span></strong>
                    @endif
                </div>
            @elseif(isset($price))
                <div class="text-center leading-none text-3xl uppercase md:text-4xl"><strong class="text-guitareo">Only $<span class="chosen-variant-price-float">{{ $price }}</span></strong></div>
            @endif
