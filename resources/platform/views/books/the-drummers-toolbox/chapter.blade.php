    ...
    <header id="bestBookHeader" class="fluid pb-5 pt-2 shadow">
        <div class="tw-w-full tw-mx-auto 3xl:tw-max-w-screen-3xl 4xl:tw-max-w-screen-4xl tw-px-4 md:tw-px-8">
            <div class="flex flex-row">
                <a href="/drumeo/{{ $isDigital ? 'drummers-toolbox-digital' : 'drummers-toolbox' }}"
                    aria-label="Back to All" class="tw-text-white no-decoration tiny uppercase pa-1">
            ...
            <div class="flex flex-row">
                <div class="flex flex-column">
                    <div class="flex flex-row align-center pv-3">
                        <a @if ($chapterNumber > 1) href="/drumeo/{{ $isDigital ? 'drummers-toolbox-digital' : 'drummers-toolbox' }}/{{ $chapterNumber - 1 }}" @endif
                        ...
                        <a @if ($chapterNumber < 10) href="/drumeo/{{ $isDigital ? 'drummers-toolbox-digital' : 'drummers-toolbox' }}/{{ $chapterNumber + 1 }}" @endif
                    ...
                    <redeem-login-buttons :redeem-api="'{{ url()->route('access-codes.form-claim') }}'"
                        :is-digital="{{ $isDigital ? 'true' : 'false' }}"
                        :is-user="{{ !empty($user) ? 'true' : 'false' }}"
                        :is-member="{{ !empty($user) && $user->isAMember() ? 'true' : 'false' }}"
                        book-context="drummers-toolbox" book-title="Drummer's Toolbox"></redeem-login-buttons>
                </div>
            </div>
        </div>
    </header>
    ...
    <div class="modal" id="youtubeLessonModal">
        <div class="flex flex-column bg-black corners-3">
            <div class="widescreen">
                <iframe id="youtubeLessonIframe" frameborder="0" modestbranding="1" playsinline="1" rel="0"
    ...
    <div class="tw-w-full tw-mx-auto 3xl:tw-max-w-screen-3xl 4xl:tw-max-w-screen-4xl tw-px-4 md:tw-px-8 dark:tw-text-white">
            ...
            <div class="flex flex-row">
                <div class="flex flex-column grow">
                    @foreach ($relatedLessons as $relatedLesson)
                        @include('books.partials.related-content', [
                            'relatedLesson' => [
        ...
        @if (!empty($chapterData['loops']))
            <div class="flex flex-column corners-3 shadow mb-3">
                ...
                <div class="flex flex-row">
                    <legacy-loops :loops="{{ json_encode($chapterData['loops']) }}"></legacy-loops>
                </div>
            </div>
        @endif
                ...
                <div class="flex flex-row align-v-center flex-wrap">
                            ...
                            @if ($isDigital)
                                @if (!empty($user))
                                    @if (!$user->isAMember() && $user->isPackOrChallengeOnlyOwner())
                                        ...
                                        at the top of the One Month Access Pass that you received with your copy of the
                                        book.
                                    @endif
                                @else
                                    <br><br>
                    ...
                    <div class="flex flex-column xs-12 sm-4 hide-xs-only">
                        <img id="bestBookImage"
                            ...
                            alt="Drummer's Toolbox" class="relative" style="margin-bottom:-30px;z-index:96;">
                    </div>
                </div>
            </div>
        </div>
