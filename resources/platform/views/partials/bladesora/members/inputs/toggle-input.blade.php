...
<fieldset class="tw-flex tw-flex-row tw-items-center">
    ...
    {{-- Custon Toggle --}}
    <div v-cloak class="tw-inline-flex tw-relative tw-w-[48px] mr-1 tw-rounded-full tw-ring-offset-2 focus-within:tw-ring-1 @if(!empty($disabledOption)) original-toggle tw-hidden @endif">
        {{-- Hidden Input --}}
        <input type="hidden" class="hidden-input" name="{{ $inputName }}" value="{{ $checked ? 1 : 0 }}">
        {{-- Checkbox Input --}}
        <input tabindex="0" type="checkbox" name="{{ $inputName }}" id="{{ $inputID }}" class="tw-peer tw-absolute tw-h-0 tw-w-0 tw-top-[10px] tw-left-[10px]" {{ !empty($checked) && $checked === true ? 'checked' : '' }}>
    ...
    @if(!empty($disabledOption))
        <div v-cloak class="tw-inline-flex tw-relative tw-w-[48px] mr-1 tw-rounded-full tw-ring-offset-2 focus-within:tw-ring-1 tw-hidden disabled-toggle">
