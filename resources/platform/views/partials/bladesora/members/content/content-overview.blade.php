...
<div
    class="flex flex-row pv-3 content-overview {{ !empty($hideBorder) && $hideBorder ? '' : 'tw-border-b tw-border-[#D4D4D8] dark:tw-border-[#223F57]' }}">
    <div
        class="flex flex-column align-v-center large-thumbnail {{ !empty($showBackgroundCards) && $showBackgroundCards === true ? 'background-cards' : '' }} {{ $themeColor }} {{ !empty($releaseDate) && \Carbon\Carbon::parse($releaseDate) > \Carbon\Carbon::now() ? 'desaturate' : '' }}">
                ...
                <div
                    ...
                    @if (!empty($logoImage))
                        <div class="logo-image pa-1 corners-bottom-3">
                            <img src="{{ $logoImage }}" alt="{{ $itemTitle }} Logo">
                        </div>
                    @endif
        ...
        <div
            ...
            <div class="flex flex-column align-v-center grow ph p-sm-up">
                ...
                @if (isset($includedWithEdge) && $includedWithEdge === true)
                    <p class="tiny tw-uppercase tw-flex tw-items-center tw-text-[#3F3F46] dark:tw-text-[#9EC0DC]"
                        style="margin-bottom: 8px;">
                        <svg width="13" height="13" viewBox="0 0 13 13" fill="none"
                            xmlns="http://www.w3.org/2000/svg" class="tw-mr-1">
                            <path
                                d="M5.80262 0.917433L4.21589 4.02017L0.665805 4.51933C0.0291704 4.60838 -0.225969 5.36532 0.235712 5.79886L2.80412 8.21263L2.19664 11.6224C2.0873 12.2387 2.76038 12.7004 3.32412 12.4121L6.5 10.8021L9.67588 12.4121C10.2396 12.698 10.9127 12.2387 10.8034 11.6224L10.1959 8.21263L12.7643 5.79886C13.226 5.36532 12.9708 4.60838 12.3342 4.51933L8.78411 4.02017L7.19738 0.917433C6.91308 0.364376 6.08935 0.357346 5.80262 0.917433Z"
                                fill="currentColor" />
                        </svg>
                        Included With Edge
                    </p>
                ...
                @if ($itemTitle === '30-Day Drummer')
                    <div class="tw-mb-2">
                        <p
                            class="tw-uppercase tiny tw-rounded-lg tw-font-bold tw-py-1 tw-px-2.5 tw-inline-block tw-bg-[#000C17] tw-text-white dark:tw-bg-white dark:tw-text-black">
                            September 2022</p>
                    </div>
                @endif
            ...
            @if (!empty($itemDetails) && $isOwned)
                <div
                    class="flex flex-column align-center basic-col tw-uppercase text-grey-3 dark:tw-text-[#9EC0DC] font-italic tw-text-xs hide-sm-down">
                    {{ $itemDetails }}
                </div>
            @endif
            ...
            @if (!$isOwned && \Carbon\Carbon::parse($releaseDate) < \Carbon\Carbon::now())
                <div class="flex flex-column buy-col align-center">
                    <a href="/#customize-anchor" target="_blank"
                    ...
                    <p class="tiny font-bold uppercase text-black dark:text-white dense">
                        <a href="{{ $salesUrl }}" target="_blank" class="text-black">Or Get a Single Learning
                            Path</a>
                        ($97)
                    </p>
                </div>
            ...
            @if ($isOwned || \Carbon\Carbon::parse($releaseDate) > \Carbon\Carbon::now())
                <div class="lg:tw-flex tw-flex-row icon-col align-v-center tw-hidden">
                    <button id="addToPlaylistBtn" data-payload="{{ $parsedPayload }}" class="tw-mr-[25px]">
                        <i class="fas fa-plus flex-center text-grey-2 dark:tw-text-[#9EC0DC] rounded"></i>
                    </button>
                    <div class="body {{ \Carbon\Carbon::parse($releaseDate) > \Carbon\Carbon::now() ? 'addeventatc' : '' }}"
                        data-dropdown-y="up" data-dropdown-x="right" data-intel-apple="true">
                        @if (\Carbon\Carbon::parse($releaseDate) < \Carbon\Carbon::now())
                            <a @if (empty($noLink) || $noLink === false) href="{{ $lessonsUrl }}" @endif
                                class="no-decoration">
                                @if (!empty($itemProgress))
                                    <i
                                        class="fas {{ $itemProgress == 'completed' ? 'fa-check-circle' : 'fa-adjust' }} flex-center text-{{ $themeColor }} rounded"></i>
                                @else
                                    <i
                                        class="fas fa-arrow-circle-right flex-center text-grey-2 dark:tw-text-[#9EC0DC] rounded"></i>
                                @endif
                            </a>
                        @else
