<template>
    <div class="text-editor-container tw-flex tw-flex-col tw-w-full" v-if="renderTinyMCE">
        ...
        <tinymce-editor v-model="contentInterface" api-key="g84168rl7b45du7fji2nive374o541mhtmzogyolgqng97xc"
            :init="initObject" @change="handleInput" :placeholder="placeholder"></tinymce-editor>
    </div>
</template>
<script>
...
export default {
        ...
        initObject() {
            return {
                ...
                setup: (editor) => {
                    ...
                    editor.on('drop', this.handleEditorDrop);
                },
            };
        },
    },
    methods: {
        ...
        handleEditorDrop(event) {
            ...
            console.log(files);
            if (files.length) {
                //const newFileName = `${this.userId}_${Date.now()}_${this.commentId}_.png`;
...
/*
                axios.post(this.imageUploadEndpoint, formData)
                    .then((response) => {
                        console.log(response);
                        editor.insertContent(`<img src="${response.data.imageUrl}" />`);
                    })
                    .catch((error) => {
