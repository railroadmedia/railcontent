<!-- Composition API -->
<script setup>
import { ref, onMounted, beforeUnmount, computed } from 'vue';
    ...
    */
    ...
    if (shaka.Player.isBrowserSupported() && !PlayerUtils.isIE()) {
        ...
        Object.keys(this.eventHandlers).forEach((event) => {
            shakaPlayer.addEventListener(event, this.eventHandlers[event]);
        });
    ...
    chromeCast.value.on('available', () => {
        ...
        Object.keys(this.chromeCastEventHandlers).forEach((event) => {
            chromeCast.value.on(event, this.chromeCastEventHandlers[event]);
        });
