...
const buildDescription = (yearUpgradeCost) => {
    if (yearUpgradeCost && yearUpgradeCost > 0) {
        descriptions.value = {
            month: `By clicking "Pay Monthly", I agree to be billed immediately for a prorate upgrade cost of ${yearUpgradeCost}. I understand that my next renewal will be $30 for Musora+ with Songs.`,
            year: `By clicking "Pay Annually", I agree to be billed immediately for a prorate upgrade cost of $${yearUpgradeCost}. I understand that my next renewal will be $240 for Musora+ with Songs.`
        };
    } else {
        descriptions.value = {
...
const handleSelectedInterval = () => {
};

onMounted(() => {
    axios.get('/ecommerce/subscription/change/info').then((r) => {
        console.log(r.data)
        const { yearUpgradeCost, selectedTier } = r.data;
        buildDescription(yearUpgradeCost);
    }).catch(() => {
        window.shownotification({
            icon: 'error',
            text: 'There was an error saving your info'
        });
    });
});
            ...
            <div class="tw-text-center tw-text-white tw-px-5 sm:tw-px-6 tw-py-10 sm:tw-py-14 lg:tw-py-20 tw-bg-[#081825] tw-border-[#445F74] tw-rounded-[10px] tw-border-[1px]">
                ...
                <div class="tw-flex tw-items-end tw-justify-center">
                    <button @click="() => handleSelectedTier('plus')" class="tw-px-2 tw-relative">
                        ...
                        <div class="tw-box-border tw-relative tw-flex tw-items-end tw-rounded-xl tw-px-3 tw-py-2 tw-w-[198px] tw-h-[60px]"
                    ...
                    <button @click="() => handleSelectedTier('basic')" class="tw-px-2 tw-relative">
                        <div class="tw-box-border tw-flex tw-items-end tw-rounded-xl tw-px-3 tw-py-2 tw-w-[198px] tw-h-[60px]"
                            :class="`${selectedTier === 'basic' ? 'tw-border-2 tw-border-white' : ''}`"
                            style="background-color:#273040;">
                            <div class="tw-text-left">
                                <img class="tw-w-[79px]"
                                    src="https://dmmior4id2ysr.cloudfront.net/logos/musora-logo-white.png">
                                <p class="tw-opacity-60 tw-text-[10px]"><em>Lessons only.</em></p>
                            </div>
                            <div
                    ...
                    <div class="tw-flex tw-flex-col tw-w-full tw-h-full md:tw-w-1/2 tw-px-2 md:tw-px-3 tw-relative">
                        ...
                        <div class="tw-flex tw-flex-col tw-h-full tw-text-black tw-rounded-2xl tw-mx-auto tw-mb-4 md:tw-mb-0 tw-group">
                            <div class="tw-bg-white tw-px-3 tw-pt-6 md:tw-pt-8 tw-pb-5 md:tw-pb-8">
                                ...
                                <p class="tw-text-sm tw-mb-4"><em>{{ intervalTierPrices[selectedTier].year.description }}</em></p>
                            ...
                            <div class="tw-grow tw-h-full tw-px-3 tw-pt-5 md:tw-pt-6 tw-pb-9 md:tw-pb-10"
                                ...
                                <p class="tw-mb-2" v-if="selectedTier === 'plus'"><strong>Thousand of popular songs</strong></p>
                                <p class="tw-mb-1">{{ selectedTier === 'plus' ? descriptions.year : DEFAULT_DESCRIPTION }}</p>
                            </div>
                        </div>
                    </div>
                    <div class="tw-flex tw-flex-col tw-grow tw-w-full tw-h-full md:tw-w-1/2 tw-px-2 md:tw-px-3 tw-relative">
                        <div class="tw-flex tw-flex-col tw-grow tw-text-black tw-rounded-2xl tw-mx-auto tw-mb-4 md:tw-mb-0 tw-group">
                            <div class="tw-bg-white tw-px-3 tw-pt-6 md:tw-pt-8 tw-pb-5 md:tw-pb-8">
                                ...
                                <p class="tw-text-sm tw-mb-4"><em>{{ intervalTierPrices[selectedTier].month.description }}</em></p>
                            ...
                            <div class="tw-grow tw-px-3 tw-pt-5 md:tw-pt-6 tw-pb-9 md:tw-pb-10"
                                ...
                                <p class="tw-mb-1"><strong>Unlimited personal support</strong></p>
                                <p class="tw-mb-2" v-if="selectedTier === 'plus'"><strong>Thousand of popular songs</strong></p>
                                <p class="tw-mb-1">{{ selectedTier === 'plus' ? descriptions.month : DEFAULT_DESCRIPTION }}</p>
                            </div>
                        </div>
                    </div>
                </div>
