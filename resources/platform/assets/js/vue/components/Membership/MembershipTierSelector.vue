...
<template>
    <div class="tw-flex tw-items-end tw-justify-center">
        <button @click="() => emit('onTierSelect', 'plus')" class="tw-px-2 tw-relative">
            <strong
            ...
            <div class="tw-box-border tw-relative tw-flex tw-items-end tw-rounded-xl tw-px-3 tw-py-2 tw-w-[160px] lg:tw-w-[198px] tw-h-[60px]"
                ...
                <div class="tw-flex tw-h-full tw-absolute tw-right-0 tw-items-center tw-justify-end -tw-mb-2 tw-mr-2">
                    <i :class="`${selectedTier === 'plus' ? 'fas fa-check-circle' : 'far fa-circle'} tw-text-2xl`"></i>
                </div>
            </div>
        </button>
        <button @click="() => emit('onTierSelect', 'basic')" class="tw-px-2 tw-relative">
