    ...
    <div class="tw-flex tw-flex-col tw-pr-0 xl:tw-pr-8 tw-grow tw-w-full">
        <div v-if="!isLoading" class="tw-flex tw-flex-col sm:tw-flex-row tw-py-4">
            <div class="tw-flex tw-flex-col sm:tw-mr-6 tw-mb-6 sm:tw-mb-0">
                <div
                    class="tw-flex tw-flex-shrink-0 tw-items-center tw-justify-center tw-aspect-square tw-w-full tw-min-w-[175px] sm:tw-max-w-[338px]  2xl:tw-w-screen tw-relative tw-overflow-hidden tw-rounded-[10px] tw-bg-white dark:tw-bg-[#0E2031]">
                    <!-- Song Image Background -->
                    <img :src="`https://www.musora.com/cdn-cgi/image/width=500,quality=95/${thumbnailUrl}`"
                        class="tw-absolute tw-transition-opacity tw-duration-500 tw-opacity-0 tw-blur-sm" loading="lazy"
            ...
            <div class="tw-flex flex-column tw-w-full">
                <div>
                    ...
                    <ContentLessonActionButtons :brand="brand" :title="songTitle" :description="songArtist"
                        :is-liked="isLiked" :like-count="likeCount" :is-added="isAdded" :content-id="contentId"
                        :user-id="userId" :resources="resources" content-type="song" :thumbnailUrl="thumbnailUrl"
                        :report-user-email="userEmail" :report-user-name="userDisplayName" :report-logo="reportLogo" />
                </div>
            </div>
        </div>
        ...
        <div v-else>
            <section class="tw-animate-pulse tw-flex tw-flex-col sm:tw-flex-row tw-py-4">
                <div class="tw-flex tw-flex-col sm:tw-mr-6 tw-mb-6 sm:tw-mb-0">
                    <!-- Thumb Skeleton -->
                    <div
                        class="tw-flex tw-flex-shrink-0 tw-aspect-square tw-w-full tw-min-w-[175px] sm:tw-max-w-[338px] 2xl:tw-w-screen tw-rounded-[10px] tw-bg-white dark:tw-bg-[#0E2031]">
                    </div>
                </div>
                <div class="tw-flex flex-column tw-w-full">
                    <!-- Title Skeletons -->
                    ...
                    <div
                        class="tw-h-[28px] tw-mt-[.625rem] tw-mb-3 tw-flex tw-w-1/3 tw-rounded-sm tw-bg-white dark:tw-bg-[#0E2031]">
                    </div>
                    <!-- Button Skeletons -->
                    <div
                        class="tw-h-[50px] tw-flex tw-rounded-full tw-bg-white dark:tw-bg-[#0E2031] tw-mb-3 3xl:tw-mb-0 3xl:tw-mr-3">
                    </div>
                    ...
                    <div class="tw-flex tw-flex-wrap tw-items-start tw-my-2">
                        <div
                            class="tw-w-[52px] tw-h-[34px] tw-flex tw-mr-2 tw-mb-2 tw-rounded-full tw-bg-white dark:tw-bg-[#0E2031]">
                        </div>
                        <div
                            class="tw-w-[65px] tw-h-[34px] tw-flex tw-mr-2 tw-mb-2 tw-rounded-full tw-bg-white dark:tw-bg-[#0E2031]">
                        </div>
                        <div
                            class="tw-w-[65px] tw-h-[34px] tw-flex tw-mr-2 tw-mb-2 tw-rounded-full tw-bg-white dark:tw-bg-[#0E2031]">
                        </div>
                        <div
                            class="tw-w-[34px] tw-h-[34px] tw-flex tw-mr-2 tw-mb-2 tw-rounded-full tw-bg-white dark:tw-bg-[#0E2031]">
                        </div>
                    </div>
                </div>
            </section>
        ...
        <transition name="show-from-bottom">
            <div v-if="openSoundslice === 'instrumentless'" id="practiceOverlay" class="bg-white">
                <SoundSlice :user-id="userId" :theme-color="brand"
                    ...
                    :soundslice-slug="soundsliceObject.soundsliceSlug" :contentId="contentId" soundsliceType="song">
