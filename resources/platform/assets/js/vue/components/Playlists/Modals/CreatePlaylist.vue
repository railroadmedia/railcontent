...
<template>
    <div class="tw-flex tw-flex-col tw-justify-center tw-text-[#0D0D0D] dark:tw-text-white">
        ...
        <div class="tw-pt-[30px] tw-flex tw-flex-col sm:tw-flex-row tw-justify-between">
            ...
            <button @click="() => handleConfirm()" :class="`tw-mb-2 sm:tw-mb-0 sm:tw-ml-4 tw-btn-primary tw-bg-${brand} tw-text-center tw-flex tw-justify-center tw-items-center`"><span>{{ props.modalProps.modalType === 'create' ? 'create' : props.modalProps.modalType === 'duplicate' ? 'duplicate' : 'save' }}</span></button>
        </div>
    </div>
</template>
