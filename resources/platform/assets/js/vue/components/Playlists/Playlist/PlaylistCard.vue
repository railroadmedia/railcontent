    ...
    <div :id="cardId"
        ...
        <a :href="lesson.url && !showMask ? lesson.url : ''"
            ...
            <div
                ...
                <!-- Release Date -->
                <div v-if="!released"
                    class="tw-z-20 tw-absolute tw-top-0 tw-w-full tw-h-full tw-left-0 tw-bg-black/80 tw-flex tw-flex-col tw-items-center tw-justify-center">
            ...
            <div v-if="cueVersion"
                class="tw-inline-flex tw-items-center tw-justify-end tw-shrink-0 tw-w-[35px] md:tw-justify-center">
                <div class="tw-relative " v-click-outside="() => { state.dropdownOpen = false }">
                    <button v-if="!needAccess && !playlistsStore.sortingPlaylist"
                        class="tw-text-[#445F74] dark:tw-text-[#7E9AB1] tw-bg-transparent dark:hover:tw-bg-[#102230] hover:tw-bg-[#F5F5F6] tw-transition-colors tw-p-0 tw-mb-0 focus-visible:tw-outline focus-visible:tw-outline-[#111827] dark:focus:tw-outline-[#9EC0DC] focus-visible:tw-outline-2"
