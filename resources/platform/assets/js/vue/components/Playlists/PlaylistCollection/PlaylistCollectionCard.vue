...
<template>
    <div class="tw-group" 
         :class="props.isListView ? 'tw-h-[52px] tw-flex tw-flex-row tw-w-full tw-items-center tw-py-0.5 dark:hover:tw-bg-[#081825]/50 dark:even:tw-bg-[#081825]' : 'tw-grid tw-grid-rows-5 tw-grid-cols-5 tw-gap-1' "
    >
        <!-- Playlist thumbnail -->
        <a :href="list.url"
        ...
        >
            ...
            <div v-if="!props.isListView" 
                 class="tw-h-full tw-w-full tw-absolute tw-top-0 tw-left-0 tw-transition-colors group-hover:tw-bg-black/40">
            </div>
        </a>  
        ...
        >
            <div :class="props.isListView ? 'tw-w-full tw-grid tw-grid-cols-10 tw-items-center' : '' ">
                ...
                <!--category / duration -->
                <div class="tw-text-[#00101D] dark:tw-text-[#9EC0DC] tw-text-sm tw-flex" 
            ...
            <!-- Tack Icon -->
            <div class="tw-inline-flex tw-items-center tw-mx-4 xl:tw-mx-8" :class="[!props.isListView ? 'tw-hidden' : '',`tw-text-${brand}` ]">
                <musora-icon icon-name="tack" class="tw-w-[24px] tw-mx-auto tw-hidden md:tw-flex" />
            </div>
        </a>
        <!-- Playlist Options -->
        ...
        >
            ...
            >
                <button class="tw-btn-primary tw-btn-small tw-btn-circle tw-bg-transparent dark:hover:tw-bg-[#102230] hover:tw-bg-[#F5F5F6] tw-transition-colors tw-p-0 tw-mb-0 focus-visible:tw-outline focus-visible:tw-outline-[#111827] dark:focus:tw-outline-[#9EC0DC] focus-visible:tw-outline-2 tw-rotate-0" @click.prevent="state.dropdownOpen = !state.dropdownOpen" :class="props.isListView ? 'md:tw-rotate-90' : ''">
