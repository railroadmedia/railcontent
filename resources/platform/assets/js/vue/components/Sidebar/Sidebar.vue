    ...
    <section v-for="(section, i) in this.sidebarNavigationLinks" :key="i"
      ...
      <ul>
        <li v-for="(link, j) in section" :key="j" :class="[activePath(link.path) ? textColor[brand] : '']">
          <a :href="`${link.path}`" :target="link.name === 'Shop' ? '_blank' : '_self'"
            ...
            <musora-icon :icon-name="activePath(link.path) ? `${link.icon}-filled` : `${link.icon}`"
              class="tw-w-[24px] tw-mx-4" />
            <span class="tw-transition tw-whitespace-nowrap"
