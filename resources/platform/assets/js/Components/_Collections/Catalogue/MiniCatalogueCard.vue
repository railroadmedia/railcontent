<template>
    <div
        ...
        <div class="tw-flex tw-flex-col tw-justify-between tw-pl-[5px] tw-relative tw-h-[78px]">
            <button
                @click="$emit('addToList', { content_id: item.id, type: item.type, name: mappedData.black_title, description: mappedData.description, thumbnail_url: mappedData.thumbnail })">
                <PlusIcon class="tw-h-[18px] tw-w-[18px] dark:tw-text-white" />
            </button>
            <button
                class="tw-h-[18px] tw-w-[18px] tw-flex tw-items-center tw-justify-center tw-text-center dark:tw-text-white"
                @click="$emit('progressReset', { content_id: item.id })">
