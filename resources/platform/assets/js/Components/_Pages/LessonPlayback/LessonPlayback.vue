...
const fetchLessonData = async () => {
    if (isChallenge.value) {
        ...
        nextPreviousLessons.value = dataResult.status === 'fulfilled' ? {
            nextLesson: (!(dataResult.value.next_lesson?.unlock_date ?? '') || new Date(dataResult.value.next_lesson.unlock_date.slice(0, 19))) <= new Date() ? dataResult.value.next_lesson : null,
