...
//All Songs
export function buildQuery(type, orderBy, fields, filters) {
  ...
  // Combine the type, filter, and ordering into a single query
  const query = `*[_type == '${type}'${filterString ? ` && ${filterString}` : ''}]|order(${orderBy}){${fieldsString}}`;

  return query;
}

export async function fetchAllSongs(type = 'song', orderBy = 'artist', fields = ['_id', 'artist', 'title'], filters = []) {
  const projectId = '4032r8py'; // Your project ID
  const dataset = 'production'; // Your dataset name
  const version = '2021-06-07'; // API version
  const query = buildQuery(type, orderBy, fields, filters);
