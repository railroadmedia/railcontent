...
class CommentRepository extends RepositoryBase
{
                ...
                $this->query()
                    ...
                    ->addSelect(
                        $this->databaseManager->raw(
                            'GREATEST(COALESCE(child_comment.created_on, "0000-00-00 00.00:00"), COALESCE('.
                            ConfigService::$tableComments.
                            '.created_on, "0000-00-00 00.00:00")) as replied_on'
                        )
                    )
                    ->leftJoin(
                    ...
                    ->where(function (Builder $builder) {
                        $builder->where('child_comment.id', '=', function (Builder $builder) {
     ...
     * @param $threadRows - if any of the current user comments are replies, the threads (parent comment and other
     *     replies to it)
     *
     * @return array
     */
    ...
    private function getThreadByCommentIds(array $threadIds)
    {
        return $this->query()
            ...
            ->where(function ($query) use ($threadIds) {
                $query->whereIn('parent_id', $threadIds)
