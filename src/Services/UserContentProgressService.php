...
use Railroad\Railcontent\Events\UserContentProgressCompleted;
...
class UserContentProgressService
{
    ...
    do {
      ...
      foreach ($children as $child) {
        if ($child["state"] != self::STATE_COMPLETED) {
          ...
          event(new UserContentProgressCompleted($userId, $contentId, 100));
        }
      }
  ...
  public function saveContentProgress($contentId, $progress, $userId, $overwriteComplete = false)
  {
    ...
    if (
      !$overwriteComplete && $currentProgress &&
