...
class ShopController extends BaseController
{
    ...
    {
        $products = Product::whereHas('brand', fn($query) => $query->where('name', 'drumeo'))->where('visible', '=', 1)->where('product_type_id', '!=', 6)->where('deleted_at', null)->orderBy('display_order')->get();
    ...
    {
        $product = Product::where('slug', 'Drumeo-'.$slug)->where('deleted_at', null)->firstOrFail();
