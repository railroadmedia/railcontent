...
class ShopController extends BaseController
{
    ...
    {
        $products = Product::whereHas('brand', fn($query) => $query->where('name', 'pianote'))->where('visible', '=', 1)->where('product_type_id', '!=', 6)->where('deleted', 0)->orderBy('display_order')->get();
    ...
    {
        $product = Product::where('slug', 'Pianote-'.$slug)->where('deleted', 0)->firstOrFail();
