...
class EvaluateOrderJobManager implements ShouldQueue
{
    use Batchable;
    use Dispatchable;
    ...
    public function __construct(
        ...
        protected CarbonImmutable $startProcessedAt,
        protected CarbonImmutable $endProcessedAt
    ) {
    }
    ...
    private function getOrderIds(?string $endCursor): Collection
    {
        ...
        $gql = <<<GQL
            query {
                orders(first: $count, $cursor query: "processed_at:>=\"{$this->startProcessedAt->toIso8601String()}\" AND processed_at:<=\"{$this->endProcessedAt->toIso8601String()}\"$createdAtStartQuery$createdAtEndQuery AND status:any", sortKey: PROCESSED_AT) {
                    nodes {
                        ... on Order {
                            id
                        }
