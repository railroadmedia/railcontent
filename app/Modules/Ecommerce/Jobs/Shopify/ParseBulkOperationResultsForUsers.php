...
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels, Batchable, LogsShopify, FindsCustomersForUsers, SavesShopifyIdOnAddresses;
    ...
    public function handle(CustomerRepository $customerRepository, AddressRepository $addressRepository, EcommerceEntityManager $entityManager): void
    {
        ...
        try {
            if (app()->environment("local", "development")) {
                if (Storage::missing($this->resultsFilename)) {
                    throw new Exception(sprintf("%s: Results file %s was not found in storage and cannot be used to update our users",
                        $this->getClassName(), $this->resultsFilename));
                }
                $resultsFile = Storage::get($this->resultsFilename);
            } else {
                if (Storage::disk('musora_web_platform_s3')->missing($this->resultsFilename)) {
                    throw new Exception(sprintf("%s: Results file %s was not found in storage and cannot be used to update our users",
                        $this->getClassName(), $this->resultsFilename));
                }
                $resultsFile = Storage::disk('musora_web_platform_s3')->get($this->resultsFilename);
            }
        ...
        try {
            ...
            if ($customerData->addresses)
            {
                ...
                foreach($customerData->addresses as $address) {
                    $matchedAddresses = $this->findAddressesWithMatchingData($userId, null, $customers,
                    ...
                    if ($matchedAddresses->isEmpty()) {
                        ...
                        try {
                            ...
                            $this->logInfo(sprintf("%s: Address(es) %s synced with Shopify ID %s",
                                $this->getClassName(), $matchedAddresses->pluck("id")->implode(", "), $addressShopifyId));
    ...
    protected function getSourceFileUsers(): array
    {
        ...
        if (empty($this->_sourceFileUsers)) {
            ...
            if (app()->environment("local", "development")) {
                if (Storage::missing($this->sourceFilename)) {
                    throw new Exception(sprintf("%s: Source file %s was not found in storage and cannot be used to update our users",
                        $this->getClassName(), $this->sourceFilename));
                }
                $sourceFile = Storage::get($this->sourceFilename);
            } else {
                if (Storage::disk('musora_web_platform_s3')->missing($this->sourceFilename)) {
                    throw new Exception(sprintf("%s: Source file %s was not found in storage and cannot be used to update our users",
                        $this->getClassName(), $this->sourceFilename));
                }
                $sourceFile = Storage::disk('musora_web_platform_s3')->get($this->sourceFilename);
            }
    ...
    /**
     * Delete our source and results files from storage
     *
     * @return void
     */
    ...
    {
        if (app()->environment("local", "development")) {
            Storage::delete($this->sourceFilename);
            Storage::delete($this->resultsFilename);
        } else {
            Storage::disk('musora_web_platform_s3')->delete($this->sourceFilename);
            Storage::disk('musora_web_platform_s3')->delete($this->resultsFilename);
        }
    }
