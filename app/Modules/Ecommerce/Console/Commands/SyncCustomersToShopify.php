...
class SyncCustomersToShopify extends Command
{
        ...
        if (!$simulate) {
            try {
                if ($isCreating) {
                    ...
                    $existingCustomerShopifyId = $user->shopify_id ?? $alreadySyncedUserCustomers->first(
                    )->getShopifyId();
    ...
    private function getPhoneNumberForUser(User $user): ?string
    {
        ...
        try {
            ...
            $this->error(
                sprintf(
                    "Unable to format phone number %s for User ID %s: %s",
                    $phoneNumber,
                    $user->id,
                    $e->getMessage()
                )
     ...
     * @return void
     */
    private function addDataForUpdatedAddresses(
     ...
     * @return void
     */
    private function addDataForNewAddresses(
