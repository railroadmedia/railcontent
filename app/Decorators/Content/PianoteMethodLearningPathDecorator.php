...
class PianoteMethodLearningPathDecorator extends TypeDecoratorBase
{
    ...
    public function decorate(Collection $contents)
    {
        ...
        $contentsOfType = $contents->where('type', 'learning-path')->where('slug', 'pianote-method');
        ...
        foreach ($contentsOfType as $contentIndex => $content) {
          ...
          //  $contentsOfType[$contentIndex]['url'] = url()->route('members.learning-path.show', [$content['slug']]);
             ...
             */
            foreach ($contentsOfType[$contentIndex]['units'] as $lessonIndex => $lesson) {
            ...
            if (empty($content['units'][$currentLevelIndex]) && (!$firstIncompleteLevel)) {
            ...
            $lessonsProgressRows =
                $this->railcontentDB()
                    ...
                    ->where('ch_1.parent_id', $firstIncompleteLevel['id'])
            ...
            foreach ($lessonsProgressRows as $childProgressRow) {
                if ($childProgressRow->state != 'completed' && $childProgressRow->progress_percent != 100) {
                    ...
                    $contentsOfType[$contentIndex]['next_lesson_rank'] = $childProgressRow->lesson_position;
                    break;
                }
            }

            $contentsOfType[$contentIndex]['next_lesson'] = $nextLesson;
